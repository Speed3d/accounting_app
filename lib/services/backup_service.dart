...
print("✅ تم التحقق من سلامة الملف بنجاح");
      ...
      // ============================================================================
      // ← Hint: 🔥 استخراج dbEncryptionKey من البيانات المفكوكة!
      // ============================================================================
      ...
      if (dbBytes.length < 1024 + 4) {
        // ← Hint: على الأقل 4 bytes للطول + بيانات
        throw Exception('الملف تالف - حجم البيانات صغير جداً');
      }

      // ← Hint: استخراج طول المفتاح (أول 4 bytes)
      final dbKeyLength = (dbBytes[0] << 24) |
          (dbBytes[1] << 16) |
          (dbBytes[2] << 8) |
          dbBytes[3];

      print("🔹 استخراج dbEncryptionKey من النسخة - الطول: $dbKeyLength bytes");

      // ← Hint: التحقق من صحة الطول
      if (dbKeyLength < 16 || dbKeyLength > 256 || dbBytes.length < 4 + dbKeyLength + 1024) {
        throw Exception('بيانات النسخة الاحتياطية تالفة - طول المفتاح غير صحيح');
      }
...
print("✅ تم التحقق من سلامة الملف بنجاح");
      ...
      // ← Hint: 🔥 استخراج dbEncryptionKey من البيانات المفكوكة!
      // ============================================================================
      // ← Hint: نفس المنطق المستخدم في extractUsersFromBackup
      // ============================================================================

      // ← Hint: التحقق من الحد الأدنى للحجم
      if (dbBytes.length < 1024 + 4) {
        throw Exception('الملف تالف - حجم البيانات صغير جداً (أقل من 1KB + 4 bytes).');
      }
      ...
      print("🔹 استخراج dbEncryptionKey - الطول: $dbKeyLength bytes");
