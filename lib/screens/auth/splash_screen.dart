...
import 'dart:async';
import 'dart:io';
import 'package:accountant_touch/screens/onboarding/onboarding_screen.dart';
import 'package:accountant_touch/screens/setup/initial_setup_screen.dart';
...
class _SplashScreenState extends State<SplashScreen> 
  ...
  Future<void> _handleNavigationWithSubscriptionCheck(
    ...
    try {
      ...
      // ═══════════════════════════════════════════════════════════════════
    // 🆕 0️⃣ أولاً: فحص Onboarding (أول مرة؟)
    // ═══════════════════════════════════════════════════════════════════

    final onboardingCompleted = await OnboardingScreen.isCompleted();
    
    if (!onboardingCompleted) {
      debugPrint('➡️ [السيناريو 0] أول مرة - عرض Onboarding');
      debugPrint('═══════════════════════════════════════════════════════════');
      _navigateToScreen(const OnboardingScreen());
      return;
    }

    debugPrint('✅ Onboarding مكتمل مسبقاً');

    // ═══════════════════════════════════════════════════════════════════
    // 🆕 0️⃣.1 ثانياً: فحص Initial Setup (تم الإعداد؟)
    // ═══════════════════════════════════════════════════════════════════

    final setupCompleted = await InitialSetupScreen.isCompleted();
    
    if (!setupCompleted) {
      debugPrint('➡️ [السيناريو 0.1] لم يتم الإعداد - عرض Initial Setup');
      debugPrint('═══════════════════════════════════════════════════════════');
      _navigateToScreen(const InitialSetupScreen());
      return;
    }

    debugPrint('✅ Initial Setup مكتمل مسبقاً');
